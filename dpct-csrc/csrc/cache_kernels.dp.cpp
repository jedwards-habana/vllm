...
/* DPCT_ORIG   cudaMemcpyKind memcpy_type;*/
  dpct::memcpy_direction memcpy_type;
  if (src_device.is_cuda() && dst_device.is_cuda()) {
    TORCH_CHECK(
      src_device.index() == dst_device.index(),
      "src and dst must be on the same GPU");
/* DPCT_ORIG     memcpy_type = cudaMemcpyDeviceToDevice;*/
    memcpy_type = dpct::device_to_device;
  } else if (src_device.is_cuda() && dst_device.is_cpu()) {
/* DPCT_ORIG     memcpy_type = cudaMemcpyDeviceToHost;*/
    memcpy_type = dpct::device_to_host;
  } else if (src_device.is_cpu() && dst_device.is_cuda()) {
/* DPCT_ORIG     memcpy_type = cudaMemcpyHostToDevice;*/
...
void copy_blocks_kernel(int64_t *key_cache_ptrs, int64_t *value_cache_ptrs,
                        const int64_t *__restrict__ block_mapping,
                        const int numel_per_block,
                        const sycl::nd_item<3> &item_ct1) {
/* DPCT_ORIG   const int layer_idx = blockIdx.x;*/
  const int layer_idx = item_ct1.get_group(2);
/* DPCT_ORIG   const int pair_idx = blockIdx.y;*/
...
/* DPCT_ORIG   for (int i = threadIdx.x; i < numel_per_block; i += blockDim.x)
 * {*/
...
/* DPCT_ORIG template<typename scalar_t, typename cache_t, bool
...
/*
DPCT1049:19: The work-group size passed to the SYCL kernel may exceed the limit.
To get the device limit, query info::device::max_work_group_size. Adjust the
work-group size if needed.
*/
#define CALL_RESHAPE_AND_CACHE(KV_T, CACHE_T, IS_FP8_E5M2_KV_CACHE)            \
   /* DPCT_ORIG   vllm::reshape_and_cache_kernel<KV_T, CACHE_T,                \
      IS_FP8_E5M2_KV_CACHE><<<grid, block, 0, stream>>>(                       \
  ...
  [&](sycl::handler &cgh) {\
    ...
    float * key_data_ptr_ct0 = reinterpret_cast<KV_T*>(key.data_ptr());\
    float * value_data_ptr_ct1 = reinterpret_cast<KV_T*>(value.data_ptr());\
    float * key_cache_data_ptr_ct2 = reinterpret_cast<CACHE_T*>(key_cache.data_ptr());\
    float * value_cache_data_ptr_ct3 = reinterpret_cast<CACHE_T*>(value_cache.data_ptr());\
...
\
    cgh.parallel_for(\
      ...
      [=](sycl::nd_item<3> item_ct1) {\
        vllm::reshape_and_cache_kernel<KV_T, CACHE_T, IS_FP8_E5M2_KV_CACHE>(key_data_ptr_ct0, value_data_ptr_ct1, key_cache_data_ptr_ct2, value_cache_data_ptr_ct3, slot_mapping_data_ptr_int64_t_ct4, key_stride_ct5, value_stride_ct6, num_heads_ct7, head_size_ct8, block_size_ct9, x_ct10, item_ct1);\
      });\
  });
...
/* DPCT_ORIG   dim3 block(std::min(num_heads * head_size, 512));*/
  sycl::range<3> block(std::min(num_heads * head_size, 512));
...
/* DPCT_ORIG       CALL_RESHAPE_AND_CACHE(__nv_bfloat16, __nv_bfloat16,
 * false);*/
      CALL_RESHAPE_AND_CACHE(sycl::ext::oneapi::bfloat16,
...
/* DPCT_ORIG       CALL_RESHAPE_AND_CACHE(__nv_bfloat16, uint8_t, true);*/
      CALL_RESHAPE_AND_CACHE(sycl::ext::oneapi::bfloat16, uint8_t, true);
    }
  } else {
...
void gather_cached_kv_kernel(
    ...
    const int head_size, const int block_size, const int x,
    const sycl::nd_item<3> &item_ct1) {
/* DPCT_ORIG     const int token_idx = blockIdx.x;*/
...
/* DPCT_ORIG     for (int i = threadIdx.x; i < num_tokens; i += blockDim.x) {*/
...
gather_cached_kv_kernel_optimized exceeds 128 bytes and may cause high register
...
/* DPCT_ORIG     for (int i = threadIdx.x; i < unrolled_dim; i += blockDim.x)*/
...
void convert_fp8_e5m2_kernel(const Tin *__restrict__ src_cache,
                             Tout *__restrict__ dst_cache,
                             const int64_t block_stride,
                             const sycl::nd_item<3> &item_ct1) {
/* DPCT_ORIG   const int64_t block_idx = blockIdx.x;*/
  const int64_t block_idx = item_ct1.get_group(2);
/* DPCT_ORIG   for (int i = threadIdx.x; i < block_stride; i += blockDim.x) {*/
...
/* DPCT_ORIG   dim3 grid(num_blocks);*/
  sycl::range<3> grid(1, 1, num_blocks);
/* DPCT_ORIG   dim3 block(std::min(block_stride, int64_t(512)));*/
  sycl::range<3> block(std::min(block_stride, int64_t(512)));
/* DPCT_ORIG   const cudaStream_t stream = at::cuda::getCurrentCUDAStream();*/
  const dpct::queue_ptr stream = at::cuda::getCurrentCUDAStream();
...
/* DPCT_ORIG     CALL_CONVERT_FP8_E5M2(uint8_t, __nv_bfloat16);*/
    CALL_CONVERT_FP8_E5M2(uint8_t, sycl::ext::oneapi::bfloat16);
  } else if (dst_cache.dtype() == at::ScalarType::Float) {
    CALL_CONVERT_FP8_E5M2(float, uint8_t);
  } else if (dst_cache.dtype() == at::ScalarType::Half) {
    CALL_CONVERT_FP8_E5M2(uint16_t, uint8_t);
  } else if (dst_cache.dtype() == at::ScalarType::BFloat16) {
/* DPCT_ORIG     CALL_CONVERT_FP8_E5M2(__nv_bfloat16, uint8_t);*/
    CALL_CONVERT_FP8_E5M2(sycl::ext::oneapi::bfloat16, uint8_t);
  }
}
