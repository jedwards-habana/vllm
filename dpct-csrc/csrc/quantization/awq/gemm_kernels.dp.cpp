...
/* DPCT_ORIG #include "dequantize.cuh"*/
...
/* DPCT_ORIG static inline __device__ __host__ unsigned
...
/* DPCT_ORIG __global__ void __launch_bounds__(64)
 * gemm_forward_4bit_cuda_m16n128k32(int G, int split_k_iters, half*
 * __restrict__ A, int* __restrict__ B, half* __restrict__ scaling_factors, int*
 * __restrict__ zeros, int M, int IC, int OC, half* __restrict__ C) */
...
{
/* DPCT_ORIG #if defined(__CUDA_ARCH__) && __CUDA_ARCH__ < 750*/
...
/* DPCT_ORIG __global__ void __launch_bounds__(64)
 * gemm_forward_4bit_cuda_m16n64k32(int G, int split_k_iters, half* __restrict__
 * A, int* __restrict__ B, half* __restrict__ scaling_factors, int* __restrict__
 * zeros, int M, int IC, int OC, half* __restrict__ C) */
...
/* DPCT_ORIG __global__ void __launch_bounds__(64) dequantize_weights(
    int* __restrict__ B,
                        ...
                        int G, const sycl::nd_item<3> &item_ct1)
{
  ...
  int split_k_iters = 1;
  static constexpr uint32_t ZERO = 0x0;
/* DPCT_ORIG   half B_shared[32 * (128 + 8)];*/
  sycl::half B_shared[32 * (128 + 8)];

/* DPCT_ORIG   half* B_shared_ptr2 = B_shared;*/
  sycl::half *B_shared_ptr2 = B_shared;

/* DPCT_ORIG   half B_shared_warp[32];*/
  sycl::half B_shared_warp[32];
  int OC = 512;

/* DPCT_ORIG   int N = blockDim.x * gridDim.x;  */
  int N = item_ct1.get_local_range(2) * item_ct1.get_group_range(2); // 2
/* DPCT_ORIG   int col = (blockIdx.x * blockDim.x + threadIdx.x);*/
  int col = (item_ct1.get_group(2) * item_ct1.get_local_range(2) +
             item_ct1.get_local_id(2));
/* DPCT_ORIG   int row = blockIdx.y * blockDim.y + threadIdx.y;*/
  int row = item_ct1.get_group(1) * item_ct1.get_local_range(1) +
            item_ct1.get_local_id(1);
  int index1 = 8 * col + 8 * row * N;
/* DPCT_ORIG   half* C_ptr2 = C + index1;*/
...
/* DPCT_ORIG   half* scaling_factors_ptr2 = scaling_factors + index4;*/
  sycl::half *scaling_factors_ptr2 = scaling_factors + index4;

    uint32_t zeros_loaded = *(uint32_t*)(zeros_ptr2);
/* DPCT_ORIG     uint4 B_loaded_zero = dequantize_s4_to_fp16x2(zeros_loaded);*/
    sycl::uint4 B_loaded_zero = dequantize_s4_to_fp16x2(zeros_loaded);
/* DPCT_ORIG     uint4 B_loaded_scale = *(uint4*)(scaling_factors_ptr2);*/
    sycl::uint4 B_loaded_scale = *(sycl::uint4 *)(scaling_factors_ptr2);
int j=0;

      uint32_t B_loaded = *(uint32_t*)(B_ptr2 + j);
/* DPCT_ORIG       uint4 B_loaded_fp16 = dequantize_s4_to_fp16x2(B_loaded);*/
      sycl::uint4 B_loaded_fp16 = dequantize_s4_to_fp16x2(B_loaded);
/* DPCT_ORIG       asm volatile("sub.f16x2 %0, %1, %2;\n" :
 * "=r"(B_loaded_fp16.x) : "r"(B_loaded_fp16.x), "r"(B_loaded_zero.x));*/
      /*
...
/* DPCT_ORIG       asm volatile("fma.rn.f16x2 %0, %1, %2, %3;\n" :
...
/* DPCT_ORIG       asm volatile("fma.rn.f16x2 %0, %1, %2, %3;\n" :
 * "=r"(B_loaded_fp16.y) : "r"(B_loaded_fp16.y), "r"(B_loaded_scale.y),
 * "r"(ZERO));*/
      /*
...
/* DPCT_ORIG       asm volatile("fma.rn.f16x2 %0, %1, %2, %3;\n" :
 * "=r"(B_loaded_fp16.z) : "r"(B_loaded_fp16.z), "r"(B_loaded_scale.z),
 * "r"(ZERO));*/
      /*
...
/* DPCT_ORIG       *(uint4*)(B_shared_ptr2 + j) = B_loaded_fp16;*/
      *(sycl::uint4 *)(B_shared_ptr2 + j) = B_loaded_fp16;
...
/* DPCT_ORIG     dim3 num_blocks(x_blocks, y_blocks);*/
    sycl::range<3> num_blocks(1, y_blocks, x_blocks);
/* DPCT_ORIG     dim3 threads_per_block(x_thread, y_thread);*/
...
/* DPCT_ORIG     vllm::awq::dequantize_weights<<<num_blocks, threads_per_block,
   0, stream>>>( kernel, scaling_factors, zeros, de_kernel, G);*/
    /*
...
/* DPCT_ORIG         dim3 num_blocks((num_out_feats + 16 - 1) / 16 * j_factors1
 * * split_k_iters);*/
        sycl::range<3> num_blocks(
            1, 1, (num_out_feats + 16 - 1) / 16 * j_factors1 * split_k_iters);
        // threadIdx.x: 32
        // threadIdx.y: i_factors[2] * j_factors[2]
/* DPCT_ORIG         dim3 threads_per_block(32, 2);*/
        sycl::range<3> threads_per_block(1, 2, 32);
/* DPCT_ORIG         vllm::awq::gemm_forward_4bit_cuda_m16n128k32<<<num_blocks,
   threads_per_block, 0, stream>>>( group_size, split_k_iters, in_feats, kernel,
   scaling_factors, zeros, num_in_feats, num_in_channels, num_out_channels,
   out_feats);*/
      {
...
/* DPCT_ORIG         dim3 num_blocks(1 * (num_out_feats + 16 - 1) / 16 *
...
/* DPCT_ORIG         dim3 threads_per_block(32, 2);*/
        sycl::range<3> threads_per_block(1, 2, 32);
/* DPCT_ORIG         vllm::awq::gemm_forward_4bit_cuda_m16n64k32<<<num_blocks,
   threads_per_block, 0, stream>>>( group_size, split_k_iters, in_feats, kernel,
   scaling_factors, zeros, num_in_feats, num_in_channels, num_out_channels,
   out_feats);*/
      {
