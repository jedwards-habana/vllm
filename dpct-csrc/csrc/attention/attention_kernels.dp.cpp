...
/* DPCT_ORIG #include "attention_utils.cuh"*/
#include "attention_utils.dp.hpp"
/* DPCT_ORIG #include "../quantization/fp8_e5m2_kvcache/quant_utils.cuh"*/
#include "../quantization/fp8_e5m2_kvcache/quant_utils.dp.hpp"
...
#pragma unroll
  for (int mask = WARP_SIZE / 2; mask >= 1; mask /= 2) {
    /*
    DPCT1096:103: The right-most dimension of the work-group used in the SYCL
...
#pragma unroll
  for (int mask = NUM_WARPS / 2; mask >= 1; mask /= 2) {
    /*
    DPCT1096:104: The right-most dimension of the work-group used in the SYCL
    kernel that calls this function may be less than "32". The function
    "dpct::permute_sub_group_by_xor" may return an unexpected result on the CPU
    device. Modify the size of the work-group to ensure that the value of the
    right-most dimension is a multiple of "32".
  ...
  /*
  DPCT1096:105: The right-most dimension of the work-group used in the SYCL
  kernel that calls this function may be less than "32". The function
...
template <typename scalar_t, typename cache_t, int HEAD_SIZE, int BLOCK_SIZE,
          int NUM_THREADS, bool IS_FP8_E5M2_KV_CACHE,
...
void paged_attention_kernel(
    ...
    sycl::local_accessor<Q_vec, 2> q_vecs, float *red_smem) {
/* DPCT_ORIG   const int seq_idx = blockIdx.y;*/
  const int seq_idx = item_ct1.get_group(1);
/* DPCT_ORIG   const int partition_idx = blockIdx.z;*/
  const int partition_idx = item_ct1.get_group(0);
/* DPCT_ORIG   const int max_num_partitions = gridDim.z;*/
...
/* DPCT_ORIG   const int thread_idx = threadIdx.x;*/
  const int thread_idx = item_ct1.get_local_id(2);
  const int warp_idx = thread_idx / WARP_SIZE;
  const int lane = thread_idx % WARP_SIZE;

/* DPCT_ORIG   const int head_idx = blockIdx.x;*/
  const int head_idx = item_ct1.get_group(2);
/* DPCT_ORIG   const int num_heads = gridDim.x;*/
...
/* DPCT_ORIG   __shared__ float red_smem[2 * NUM_WARPS];*/
...
/* DPCT_ORIG         qk_max = mask ? qk_max : fmaxf(qk_max, qk);*/
...
/* DPCT_ORIG     qk_max = fmaxf(qk_max, VLLM_SHFL_XOR_SYNC(qk_max, mask));*/
    /*
    DPCT1096:99: The right-most dimension of the work-group used in the SYCL
    kernel that calls this function may be less than "32". The function
    "dpct::permute_sub_group_by_xor" may return an unexpected result on the CPU
    device. Modify the size of the work-group to ensure that the value of the
    right-most dimension is a multiple of "32".
...
/* DPCT_ORIG     qk_max = fmaxf(qk_max, VLLM_SHFL_XOR_SYNC(qk_max, mask));*/
    /*
    DPCT1096:100: The right-most dimension of the work-group used in the SYCL
    kernel that calls this function may be less than "32". The function
    "dpct::permute_sub_group_by_xor" may return an unexpected result on the CPU
    device. Modify the size of the work-group to ensure that the value of the
    right-most dimension is a multiple of "32".
  ...
  /*
  DPCT1096:101: The right-most dimension of the work-group used in the SYCL
  kernel that calls this function may be less than "32". The function
  "dpct::select_from_sub_group" may return an unexpected result on the CPU
  device. Modify the size of the work-group to ensure that the value of the
  right-most dimension is a multiple of "32".
...
/* DPCT_ORIG     float val = __expf(logits[i] - qk_max);*/
    float val = sycl::native::exp(logits[i] - qk_max);
    logits[i] = val;
    exp_sum += val;
  }
  ...
  // Compute softmax.
/* DPCT_ORIG   const float inv_sum = __fdividef(1.f, exp_sum + 1e-6f);*/
...
#pragma unroll
    for (int mask = NUM_V_VECS_PER_ROW / 2; mask >= 1; mask /= 2) {
      /*
      DPCT1096:102: The right-most dimension of the work-group used in the SYCL
      kernel that calls this function may be less than "32". The function
...
/* DPCT_ORIG   const int seq_idx = blockIdx.y;*/
  ...
  if (num_partitions == 1) {
    ...
    const scalar_t* tmp_out_ptr = tmp_out + seq_idx * num_heads * max_num_partitions * HEAD_SIZE
                                          + head_idx * max_num_partitions * HEAD_SIZE;
/* DPCT_ORIG     for (int i = threadIdx.x; i < HEAD_SIZE; i += blockDim.x) {*/
  ...
  constexpr int NUM_WARPS = NUM_THREADS / WARP_SIZE;
/* DPCT_ORIG   const int warp_idx = threadIdx.x / WARP_SIZE;*/
  const int warp_idx = item_ct1.get_local_id(2) / WARP_SIZE;
/* DPCT_ORIG   const int lane = threadIdx.x % WARP_SIZE;*/
...
/* DPCT_ORIG   for (int i = threadIdx.x; i < num_partitions; i += blockDim.x)
 * {*/
  for (int i = item_ct1.get_local_id(2); i < num_partitions;
       i += item_ct1.get_local_range(2)) {
    const float l = max_logits_ptr[i];
    shared_max_logits[i] = l;
/* DPCT_ORIG     max_logit = fmaxf(max_logit, l);*/
...
#pragma unroll
  for (int mask = WARP_SIZE / 2; mask >= 1; mask /= 2) {
/* DPCT_ORIG     max_logit = fmaxf(max_logit, VLLM_SHFL_XOR_SYNC(max_logit,
 * mask));*/
    /*
    DPCT1096:106: The right-most dimension of the work-group used in the SYCL
    kernel that calls this function may be less than "32". The function
    "dpct::permute_sub_group_by_xor" may return an unexpected result on the CPU
    device. Modify the size of the work-group to ensure that the value of the
    right-most dimension is a multiple of "32".
    */
    max_logit = sycl::fmax(max_logit, VLLM_SHFL_XOR_SYNC(max_logit, mask));
  }
  if (lane == 0) {
 ...
 * mask));*/
    /*
    DPCT1096:107: The right-most dimension of the work-group used in the SYCL
    kernel that calls this function may be less than "32". The function
    "dpct::permute_sub_group_by_xor" may return an unexpected result on the CPU
    device. Modify the size of the work-group to ensure that the value of the
    right-most dimension is a multiple of "32".
  ...
  /*
  DPCT1096:108: The right-most dimension of the work-group used in the SYCL
  kernel that calls this function may be less than "32". The function
  "dpct::select_from_sub_group" may return an unexpected result on the CPU
  device. Modify the size of the work-group to ensure that the value of the
  right-most dimension is a multiple of "32".
...
/* DPCT_ORIG     float rescaled_exp_sum = exp_sums_ptr[i] * expf(l -
 * max_logit);*/
    float rescaled_exp_sum = exp_sums_ptr[i] * sycl::native::exp(l - max_logit);
...
/* DPCT_ORIG   const float inv_global_exp_sum = __fdividef(1.0f, global_exp_sum
 * + 1e-6f);*/
  ...
  scalar_t* out_ptr = out + seq_idx * num_heads * HEAD_SIZE + head_idx * HEAD_SIZE;
#pragma unroll
/* DPCT_ORIG   for (int i = threadIdx.x; i < HEAD_SIZE; i += NUM_THREADS) {*/
...
/*
DPCT1049:37: The work-group size passed to the SYCL kernel may exceed the limit.
...
#define LAUNCH_PAGED_ATTENTION_V1(HEAD_SIZE)                                   \
   VLLM_DevFuncAttribute_SET_MaxDynamicSharedMemorySize(                       \
       ((void *)vllm::paged_attention_v1_kernel<T, CACHE_T, HEAD_SIZE,         \
                                                ...
                                                IS_FP8_E5M2_KV_CACHE>),        \
       shared_mem_size);                                                       \
   /* DPCT_ORIG   vllm::paged_attention_v1_kernel<T, CACHE_T, HEAD_SIZE,       \
     BLOCK_SIZE, NUM_THREADS, IS_FP8_E5M2_KV_CACHE><<<grid, block,             \
     shared_mem_size, stream>>>( out_ptr, query_ptr, key_cache_ptr,            \
       value_cache_ptr,                                                        \
       num_kv_heads,                                                           \
       scale,                                                                  \
       block_tables_ptr,                                                       \
...
\
    ...
    CACHE_T * key_cache_ptr_ct2 = key_cache_ptr;\
    CACHE_T * value_cache_ptr_ct3 = value_cache_ptr;\
...
\
    cgh.parallel_for(\
      ...
      [=](sycl::nd_item<3> item_ct1) [[intel::reqd_sub_group_size(32)]] {\
        vllm::paged_attention_v1_kernel<T, CACHE_T, HEAD_SIZE, BLOCK_SIZE, NUM_THREADS, IS_FP8_E5M2_KV_CACHE>(out_ptr_ct0, query_ptr_ct1, key_cache_ptr_ct2, value_cache_ptr_ct3, num_kv_heads_ct4, scale_ct5, block_tables_ptr_ct6, context_lens_ptr_ct7, max_num_blocks_per_seq_ct8, alibi_slopes_ptr_ct9, q_stride_ct10, kv_block_stride_ct11, kv_head_stride_ct12, item_ct1, dpct_local_acc_ct1.get_pointer(), q_vecs_acc_ct1, red_smem_acc_ct1.get_pointer());\
      });\
  });
...
void paged_attention_v1_launcher(
  ...
  /*
  DPCT1083:38: The size of local memory in the migrated code may be different
...
/* DPCT_ORIG   dim3 grid(num_heads, num_seqs, 1);*/
  sycl::range<3> grid(1, num_seqs, num_heads);
/* DPCT_ORIG   dim3 block(NUM_THREADS);*/
...
/* DPCT_ORIG       CALL_V1_LAUNCHER_BLOCK_SIZE(__nv_bfloat16, __nv_bfloat16,
 * false);*/
      CALL_V1_LAUNCHER_BLOCK_SIZE(sycl::ext::oneapi::bfloat16,
                                  sycl::ext::oneapi::bfloat16, false);
    } else {
...
/* DPCT_ORIG       CALL_V1_LAUNCHER_BLOCK_SIZE(__nv_bfloat16, uint8_t, true);*/
      CALL_V1_LAUNCHER_BLOCK_SIZE(sycl::ext::oneapi::bfloat16, uint8_t, true);
    } else {
      TORCH_CHECK(false, "Unsupported data type: ", query.dtype());
    }
  } else {
...
/*
DPCT1049:39: The work-group size passed to the SYCL kernel may exceed the limit.
To get the device limit, query info::device::max_work_group_size. Adjust the
work-group size if needed.
*/
/*
DPCT1049:41: The work-group size passed to the SYCL kernel may exceed the limit.
To get the device limit, query info::device::max_work_group_size. Adjust the
work-group size if needed.
*/
#define LAUNCH_PAGED_ATTENTION_V2(HEAD_SIZE)                                   \
   /* DPCT_ORIG   vllm::paged_attention_v2_kernel<T, CACHE_T, HEAD_SIZE,       \
     BLOCK_SIZE, NUM_THREADS, IS_FP8_E5M2_KV_CACHE, PARTITION_SIZE>            \
...
\
    ...
    CACHE_T * key_cache_ptr_ct4 = key_cache_ptr;\
    CACHE_T * value_cache_ptr_ct5 = value_cache_ptr;\
...
void paged_attention_v2_launcher(
  ...
  /*
  DPCT1083:42: The size of local memory in the migrated code may be different
  from the original code. Check that the allocated memory size in the migrated
  code is correct.
  */
...
/* DPCT_ORIG   dim3 grid(num_heads, num_seqs, max_num_partitions);*/
  sycl::range<3> grid(max_num_partitions, num_seqs, num_heads);
  int shared_mem_size = std::max(logits_size, outputs_size);
  // For paged attention v2 reduce kernel.
/* DPCT_ORIG   dim3 reduce_grid(num_heads, num_seqs);*/
  sycl::range<3> reduce_grid(1, num_seqs, num_heads);
  /*
  DPCT1083:40: The size of local memory in the migrated code may be different
  from the original code. Check that the allocated memory size in the migrated
  code is correct.
  */
...
/* DPCT_ORIG       CALL_V2_LAUNCHER_BLOCK_SIZE(__nv_bfloat16, __nv_bfloat16,
 * false);*/
      CALL_V2_LAUNCHER_BLOCK_SIZE(sycl::ext::oneapi::bfloat16,
                                  sycl::ext::oneapi::bfloat16, false);
    } else {
      TORCH_CHECK(false, "Unsupported data type: ", query.dtype());
    }
...
/* DPCT_ORIG       CALL_V2_LAUNCHER_BLOCK_SIZE(__nv_bfloat16, uint8_t, true);*/
      CALL_V2_LAUNCHER_BLOCK_SIZE(sycl::ext::oneapi::bfloat16, uint8_t, true);
    } else {
      TORCH_CHECK(false, "Unsupported data type: ", query.dtype());
    }
  } else {
