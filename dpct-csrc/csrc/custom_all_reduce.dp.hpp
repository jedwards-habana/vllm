...
*/
/*
DPCT1009:92: SYCL uses exceptions to report errors and does not use the error
codes. The original code was commented out and a warning string was inserted.
You need to rewrite this code.
*/
#define CUDACHECK(cmd)                                                            \
    do {                                                                          \
        /* DPCT_ORIG     cudaError_t e = cmd; \*/                                                                              \
        dpct::err0 e = cmd;                                                       \
        /* DPCT_ORIG     if (e != cudaSuccess) { \*/                                                                              \
        if (e != 0) {                                                             \
            printf(                                                               \
                "Failed: Cuda error %s:%d '%s'\n",                                \
                __FILE__, __LINE__, /* DPCT_ORIG cudaGetErrorString(e)); \*/                                                                              \
                "cudaGetErrorString is not supported" /*cudaGetErrorString(e)*/); \
            exit(EXIT_FAILURE);                                                   \
        }                                                                         \
    } while (0)
...
/* DPCT_ORIG struct __align__(16) RankData { const void *__restrict__ ptrs[8];
 * };*/
struct __dpct_align__(16) RankData { const void *__restrict__ ptrs[8]; };
...
/* DPCT_ORIG struct __align__(alignof(T) * sz) array_t {*/
...
/* DPCT_ORIG #define DINLINE __device__ __forceinline__*/
#define DINLINE __dpct_inline__

// scalar cast functions
/* DPCT_ORIG DINLINE float upcast_s(half val) { return __half2float(val); }*/
...
template <> DINLINE sycl::half downcast_s(float val) {
/* DPCT_ORIG   return __float2half(val);*/
...
/* DPCT_ORIG DINLINE half &assign_add(half &a, half b) {*/
DINLINE sycl::half &assign_add(sycl::half &a, sycl::half b) {
/* DPCT_ORIG   a = __hadd(a, b);*/
  a = a + b;
  return a;
}
DINLINE float &assign_add(float &a, float b) { return a += b; }

/* DPCT_ORIG #if (__CUDA_ARCH__ >= 800 || !defined(__CUDA_ARCH__))*/
...
/* DPCT_ORIG __host__ __device__ constexpr uint64_t compute_flag(int ngpus) {*/
constexpr uint64_t compute_flag(int ngpus) {
...
/* DPCT_ORIG   if (blockIdx.x == 0) {*/
  if (item_ct1.get_group(2) == 0) {
/* DPCT_ORIG     if (threadIdx.x < ngpus)*/
    if (item_ct1.get_local_id(2) < ngpus)
      // simultaneously write to the corresponding byte to all other ranks.
      // Latency = 1 p2p write
/* DPCT_ORIG       sg.signals[threadIdx.x]->start.data[rank] = 255;*/
      sg.signals[item_ct1.get_local_id(2)]->start.data[rank] = 255;
/* DPCT_ORIG     else if (threadIdx.x == 32)*/
    else if (item_ct1.get_local_id(2) == 32)
      // reset
      meta->sg.end.flag = 0;
  }
...
DINLINE void end_sync(const RankSignals &sg, volatile Metadata *meta,
                      int rank, const sycl::nd_item<3> &item_ct1, int &num) {
  constexpr auto FLAG = compute_flag(ngpus);
/* DPCT_ORIG   __syncthreads();*/
  /*
  ...
  */
  item_ct1.barrier();
/* DPCT_ORIG   __shared__ int num;*/

/* DPCT_ORIG   if (threadIdx.x == 0) num = atomicAdd((int *)&meta->counter,
 * 1);*/
  if (item_ct1.get_local_id(2) == 0) num =
...
/* DPCT_ORIG   if (num == gridDim.x - 1) {*/
  if (num == item_ct1.get_group_range(2) - 1) {
/* DPCT_ORIG     if (threadIdx.x == 32) {*/
    if (item_ct1.get_local_id(2) == 32) {
      // reset in a different warp
      meta->counter = 0;
      meta->sg.start.flag = 0;
/* DPCT_ORIG     } else if (threadIdx.x < ngpus) {*/
    } else if (item_ct1.get_local_id(2) < ngpus) {
      // simultaneously write to the corresponding byte to all other ranks.
      // Latency = 1 p2p write
/* DPCT_ORIG       sg.signals[threadIdx.x]->end.data[rank] = 255;*/
      sg.signals[item_ct1.get_local_id(2)]->end.data[rank] = 255;
    }
...
/* DPCT_ORIG template <typename T, int ngpus>
__global__ void __launch_bounds__(512, 1)
    cross_device_reduce_1stage(RankData *_dp, RankSignals sg,
                               volatile Metadata *meta, T *__restrict__ result,
                               int rank, int size) {*/
template <typename T, int ngpus>
void cross_device_reduce_1stage(RankData *_dp, RankSignals sg,
                                volatile Metadata *meta, T *__restrict__ result,
                                int rank, int size,
                                const sycl::nd_item<3> &item_ct1, int &num) {
  using P = typename packed_t<T>::P;
  using A = typename packed_t<T>::A;
  // note: we don't reorder the address so the accumulation order is the same
  // for all ranks, ensuring bitwise identical results
  auto dp = *_dp;
  start_sync<ngpus>(sg, meta, rank, item_ct1);
  // do the actual reduction
/* DPCT_ORIG   for (int idx = blockIdx.x * blockDim.x + threadIdx.x; idx <
 * size;*/
  for (int idx = item_ct1.get_group(2) * item_ct1.get_local_range(2) +
                 item_ct1.get_local_id(2);
       idx < size;
       /* DPCT_ORIG        idx += gridDim.x * blockDim.x) {*/
       idx += item_ct1.get_group_range(2) * item_ct1.get_local_range(2)) {
    ((P *)result)[idx] =
        packed_reduce<P, ngpus, A>((const P **)&dp.ptrs[0], idx);
  }
  end_sync<ngpus, true>(sg, meta, rank, item_ct1, num);
}
...
__global__ void __launch_bounds__(512, 1)
    cross_device_reduce_2stage(RankData *_dp, RankSignals sg,
                               ...
                               int rank, int size) {*/
template <typename T, int ngpus>
void cross_device_reduce_2stage(RankData *_dp, RankSignals sg,
                                volatile Metadata *meta, T *__restrict__ result,
                                int rank, int size,
                                const sycl::nd_item<3> &item_ct1, int &num) {
/* DPCT_ORIG   int tid = blockIdx.x * blockDim.x + threadIdx.x;*/
  int tid = item_ct1.get_group(2) * item_ct1.get_local_range(2) +
            item_ct1.get_local_id(2);
/* DPCT_ORIG   int stride = gridDim.x * blockDim.x;*/
...
#pragma unroll
  ...
  end_sync<ngpus>(sg, meta, rank, item_ct1, num);
...
__global__ void __launch_bounds__(512, 1)
    cross_device_reduce_half_butterfly(RankData *_dp, RankSignals sg,
                                       ...
                                       int size) {*/
template <typename T, int ngpus>
void cross_device_reduce_half_butterfly(RankData *_dp, RankSignals sg,
                                        volatile Metadata *meta,
                                        T *__restrict__ result, int rank,
                                        int size,
 ...
 public:
  ...
  char *open_ipc_handle(const void *ipc_handle) try {
    ...
    if (new_handle) {
      char *ipc_ptr;
      /*
      DPCT1030:91: SYCL currently does not support inter-process communication
