...
/* DPCT_ORIG template<typename scalar_t, bool IS_NEOX>
...
inline void apply_rotary_embedding(scalar_t *__restrict__ arr,
                                   const scalar_t *__restrict__ cos_ptr,
                                   const scalar_t *__restrict__ sin_ptr,
...
/* DPCT_ORIG   for (int i = threadIdx.x; i < nq; i += blockDim.x) {*/
...
/* DPCT_ORIG   for (int i = threadIdx.x; i < nk; i += blockDim.x) {*/
...
/* DPCT_ORIG   dim3 block(std::min(num_heads * rot_dim / 2, 512));*/
